...
#include <cstring>
#include <ctime> // Get time stamps in table
#include <stdio.h>
#include <vector>
...
std::map<std::string, Neighbor> neighborTable;
...
void send_hello(int sock, const LocalInfo &info)
{
    if (info.bcast.empty())
        return;
...
void receive_hello(int sock, std::vector<Neighbor> &neighbors, const std::vector<LocalInfo> &self, time_t &now)
{
    char buf[256];
...
void myInfo_update(std::vector<LocalInfo> &myInfo)
{
    // [ Maybe change to bool, no need to update if no change ]
...
void snd_rec_msg()
{
    ...
    while (1)
    {
        ...
        [ Can be improved by send_hello() takes in each struct, not a vector of structs ]
        */
        time_t now = time(NULL);
        /* socket */
        fd_set readfds;
        ...
        {
            receive_hello(sock, neighbors, myInfo, now);
        }
        if (now - last_hello >= BROADCAST_INTERVAL)
        {
            // [ myInfo may need a checker if new info is available ]
            myInfo_update(myInfo);
            for (const auto &iface : myInfo)
            {
                if (iface.bcast.empty())
                    continue;
                send_hello(sock, iface);
            }
            last_hello = now;
        }
